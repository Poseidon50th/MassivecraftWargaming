...
export function forcedDefeatStatus(game, player) {
  ...
  if (!ownSets.length) return { status: "proven", reason: "No complete legal order set exists." };
  ...
  return {
    status: "proven",
    reason: "Every legal order set is defeated by every relevant opposing response.",
  };
}
