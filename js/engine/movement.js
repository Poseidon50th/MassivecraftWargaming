import { DIRECTIONS, UNIT_DEFS, facingFromStep } from "./constants.js";
...
function moveRange(game, unit) {
  ...
  return base + (game.terrain[unit.y][unit.x] === "road" ? 1 : 0);
}
...
export function legalMovesForUnit(game, unit) {
  ...
  while (queue.length) {
    ...
    for (const direction of Object.values(DIRECTIONS)) {
      ...
      if (!destinations.has(destinationKey) || destinations.get(destinationKey).path.length > steps) {
...
export function validateOrders(game, player, orders) {
  ...
  for (const order of orders) {
    ...
    if (order.kind === "move") {
      ...
      if (!legal) return "At least one movement order is no longer legal.";
