import { DIRECTIONS, UNIT_DEFS, facingFromStep, facingsForUnit } from "./constants.js";
...
function pathHasRiverPenalty(game, unit, path) {
  let previous = { x: unit.x, y: unit.y };
  for (const step of path) {
    if (
      game.terrain[previous.y][previous.x] === "river" ||
      game.terrain[step.y][step.x] === "river"
    ) return true;
    previous = step;
  }
  return false;
    ...
    for (const direction of Object.values(DIRECTIONS)) {
      ...
      if (!returnsToOrigin && (!destinations.has(destinationKey) || destinations.get(destinationKey).path.length > steps)) {
        ...
        destinations.set(destinationKey, {
          ...
          riverPenalty: pathHasRiverPenalty(game, unit, nextPath),
        });
      }
...
export function validateOrders(game, player, orders) {
  ...
  for (const order of orders) {
    ...
    if (!facingsForUnit(unit).includes(order.facing)) {
      return "That unit cannot use the chosen final facing.";
    }
    if (order.kind === "move") {
