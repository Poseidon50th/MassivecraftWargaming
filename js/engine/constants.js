...
export const CARDINAL_FACINGS = Object.freeze(["north", "east", "south", "west"]);
...
export const FACING_DIRECTIONS = Object.freeze({
  ...
  northeast: { dx: 1, dy: -1, glyph: "↗" },
  east: DIRECTIONS.east,
  southeast: { dx: 1, dy: 1, glyph: "↘" },
  south: DIRECTIONS.south,
  southwest: { dx: -1, dy: 1, glyph: "↙" },
  west: DIRECTIONS.west,
  northwest: { dx: -1, dy: -1, glyph: "↖" },
});

const EIGHT_DIRECTION_TYPES = new Set(["spear", "axe", "musket"]);
...
export const UNIT_DEFS = Object.freeze({
  ...
  spear: {
    ...
    initiative: 2,
    directional: true,
    description: "Projects 3 control into a three-square forward wedge and may face in eight directions.",
  },
  axe: {
    name: "Axes",
    short: "AX",
    ...
    initiative: 2,
    directional: true,
    description: "Projects 5 control into the square directly ahead and may face in eight directions.",
  },
  cavalry: {
    name: "Cavalry",
    short: "CV",
  ...
  musket: {
    ...
    initiative: 1,
    directional: true,
    description: "Projects 3, 2, then 1 control along a three-square firing line and may face in eight directions.",
  },
  artillery: {
    name: "Artillery",
    short: "AR",
...
export function facingsForUnit(unitOrType) {
  const type = typeof unitOrType === "string" ? unitOrType : unitOrType?.type;
  return EIGHT_DIRECTION_TYPES.has(type) ? FACINGS : CARDINAL_FACINGS;
}
...
export function rotateFacing(facing, amount = 1, unitOrType = null) {
  const available = unitOrType
    ? facingsForUnit(unitOrType)
    : CARDINAL_FACINGS.includes(facing) ? CARDINAL_FACINGS : FACINGS;
  const index = available.indexOf(facing);
  return available[(index + amount + available.length) % available.length];
}
