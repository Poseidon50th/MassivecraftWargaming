...
function blankTerrain(size, terrainEntries = []) {
  const terrain = Array.from({ length: size }, () => Array(size).fill("plain"));
...
export function createGame(scenario) {
  return {
    ...
    phase: "deployment",
    round: 0,
    terrain: blankTerrain(scenario.size, scenario.terrain),
    units: [],
    reserves: {
