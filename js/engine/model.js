...
export function createReserve(player, army) {
...
export function createGame(scenario, armies = {}) {
  const humanArmy = armies.human ?? scenario.army;
  const computerArmy = armies.computer ?? scenario.army;
  return {
    version: 4,
    ...
    units: [],
    reserves: {
      [PLAYERS.HUMAN]: createReserve(PLAYERS.HUMAN, humanArmy),
      [PLAYERS.COMPUTER]: createReserve(PLAYERS.COMPUTER, computerArmy),
    },
...
export function replaceReserve(game, player, army) {
  if (activeUnits(game, player).length) throw new Error("A deployed army cannot be replaced.");
  game.reserves[player] = createReserve(player, army);
  return game.reserves[player];
}
