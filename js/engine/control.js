import { DIRECTIONS, FACING_DIRECTIONS, PLAYERS, UNIT_DEFS, oppositePlayer } from "./constants.js";
...
function forwardBasis(facing) {
  const forward = FACING_DIRECTIONS[facing];
  return { forward, right: { dx: -forward.dy, dy: forward.dx } };
}

function spearOffsets(facing) {
  const forward = FACING_DIRECTIONS[facing];
  if (forward.dx && forward.dy) {
    return [
      { dx: 0, dy: forward.dy },
      forward,
      { dx: forward.dx, dy: 0 },
  ...
  return [-1, 0, 1].map((lateral) => ({
    dx: forward.dx + right.dx * lateral,
    dy: forward.dy + right.dy * lateral,
  }));
}
...
function rawPattern(unit) {
  ...
  if (unit.type === "sword" || unit.type === "cavalry") {
    ...
    for (const { dx, dy } of spearOffsets(unit.facing)) {
      addProjection(targets, x + dx, y + dy, 3);
    }
  } else if (unit.type === "axe") {
    const { dx, dy } = FACING_DIRECTIONS[unit.facing];
...
function artilleryDisputed(game, artillery, provisional) {
...
export function computeControlDetails(game, units = game.units) {
  ...
  const projectUnit = (unit, includeArtillery, artilleryEnabled = true) => {
    if (unit.type === "artillery" && !includeArtillery) return;
    if (unit.type === "artillery" && !artilleryEnabled) return;
  ...
  // Determine every Artillery battery's suppression from the same provisional
  // non-Artillery control map. Applying one battery before checking another
  // would make the result depend on roster order and could silence only the
  // computer or only the player in an otherwise mirrored position.
  const artilleryStates = living
    .filter((unit) => unit.type === "artillery")
    .map((unit) => ({ unit, enabled: !artilleryDisputed(game, unit, grid) }));
  artilleryStates.forEach(({ unit, enabled }) => projectUnit(unit, true, enabled));
