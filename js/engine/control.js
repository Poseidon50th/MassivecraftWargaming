...
export function computeControlDetails(game, units = game.units) {
  ...
  // Self-control is applied first so Artillery can determine whether its tile is disputed.
  for (const unit of living) {
    const modifier = sourceModifier(game, unit);
    addControl(unit, unit.x, unit.y, Math.max(0, UNIT_DEFS[unit.type].strength + modifier));
  }
