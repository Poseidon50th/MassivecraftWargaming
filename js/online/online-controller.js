import { MultiplayerApi } from "./api-client.js?v=0.8.0";
...
const elements = {
  ...
  spectateForm: $("#spectate-room-form"),
  spectateRoomCode: $("#spectate-room-code"),
  room: $("#room-screen"),
  roomCode: $("#room-code"),
  ...
  armyScreen: $("#army-screen"),
  armySelectionTitle: $("#army-selection-title"),
  armyRoomCode: $("#army-room-code"),
  armyConnectionBadge: $("#army-connection-badge"),
  armyBuilderCard: $("#army-builder-card"),
  armyBuilder: $("#online-army-builder"),
  armyLimitHeading: $("#army-limit-heading"),
  armySelectionCopy: $("#army-selection-copy"),
  lockArmy: $("#lock-army-button"),
  withdrawArmy: $("#withdraw-army-button"),
  armyYouName: $("#army-you-name"),
  armyYouReady: $("#army-you-ready"),
  armyOpponentName: $("#army-opponent-name"),
  armyOpponentReady: $("#army-opponent-ready"),
  armyWaitingCopy: $("#army-waiting-copy"),
  ...
  spectatorStatusCopy: $("#spectator-status-copy"),
  humanControlLabel: $("#human-control-label"),
  computerControlLabel: $("#computer-control-label"),
  perspectiveNote: $("#perspective-note"),
...
let spectatorActivated = false;
let armyBuilder = null;
let armyBuilderSize = null;
let armyDraft = defaultArmyForSize(8);
let armyDraftError = null;
...
function credentialsFromLocation() {
  ...
  if (!roomId || !["host", "guest", "spectator"].includes(seat)) return null;
  ...
  return {
    ...
    seat,
    token,
    guestToken: value?.guestToken ?? null,
    spectatorToken: value?.spectatorToken ?? null,
    spectatorActive: Boolean(value?.spectatorActive),
  };
}
...
function setConnection(status) {
  ...
  for (const badge of [elements.connectionBadge, elements.armyConnectionBadge, elements.battleConnectionBadge]) {
...
function inviteUrl(seat = "guest") {
  const invitationToken = seat === "spectator" ? credentials?.spectatorToken : credentials?.guestToken;
  ...
  if (localServer) url.searchParams.set("server", localServer);
  url.hash = `token=${encodeURIComponent(invitationToken)}`;
  return url.toString();
}
...
function renderEntry() {
  ...
  const spectating = credentials?.seat === "spectator" && credentials.roomId;
  elements.createForm.hidden = Boolean(invited || spectating);
  elements.joinForm.hidden = !invited;
  elements.spectateForm.hidden = !spectating;
  if (invited) elements.joinRoomCode.textContent = credentials.roomId;
  if (spectating) elements.spectateRoomCode.textContent = credentials.roomId;
}

function renderRoom() {
  const spectator = roomState.seat === "spectator";
  elements.roomCode.textContent = roomState.roomId;
  elements.youRoleLabel.textContent = spectator ? "Host" : "You";
  elements.opponentRoleLabel.textContent = spectator ? "Guest" : "Opponent";
  elements.youName.textContent = spectator ? roomState.players.host.name : (roomState.players.you.name ?? "Joining…");
  elements.opponentName.textContent = spectator
    ? (roomState.players.guest.name ?? "Waiting for invitation…")
    : (roomState.players.opponent.name ?? "Waiting for invitation…");
  elements.youConnection.classList.toggle("connected", spectator ? roomState.players.host.connected : roomState.players.you.connected || connectionStatus === "connected");
  elements.opponentConnection.classList.toggle("connected", spectator ? roomState.players.guest.connected : roomState.players.opponent.connected);
  elements.spectatorCountRow.hidden = roomState.seat === "guest";
  elements.spectatorCount.textContent = `${roomState.spectatorCount} spectator${roomState.spectatorCount === 1 ? "" : "s"}`;
  elements.invitePanel.hidden = roomState.seat !== "host";
  if (roomState.seat === "host") {
    elements.inviteLink.value = inviteUrl("guest");
    elements.spectatorLink.value = inviteUrl("spectator");
  }
  ...
  if (!proposal) {
    elements.roomTitle.textContent = spectator
      ? "Waiting for the commanders to prepare the battlefield"
      : roomState.seat === "host"
  ...
  if (proposal.status === "pending") {
    elements.roomTitle.textContent = spectator
      ? "The guest is reviewing the battlefield"
      : roomState.seat === "host" ? "Waiting for battlefield approval" : "Approve the host’s battlefield";
    elements.proposalCopy.textContent = spectator
      ? "You may inspect the proposed field. Only the invited guest can approve or decline it."
      : roomState.seat === "host"
...
function ensureArmyBuilder(size) {
  if (armyBuilder && armyBuilderSize === size) return;
  armyBuilderSize = size;
  armyDraft = defaultArmyForSize(size);
  armyBuilder = createArmyBuilder(elements.armyBuilder, {
    size,
    army: armyDraft,
    onChange: (army, state) => {
      armyDraft = army;
      armyDraftError = state.error;
      elements.lockArmy.disabled = Boolean(state.error) || busy || Boolean(roomState?.armySelection?.youReady);
    },
  });
}
...
function renderArmySelection() {
  const spectator = roomState.seat === "spectator";
  const scenario = getScenario(roomState.proposal.scenarioId);
  elements.armyRoomCode.textContent = roomState.roomId;
  elements.armySelectionTitle.textContent = spectator ? "The commanders are choosing their forces" : "Choose your force";
  elements.armyBuilderCard.hidden = spectator;
  if (spectator) {
    elements.armyWaitingCopy.textContent = `The host is ${roomState.armySelection.hostReady ? "ready" : "choosing"}; the guest is ${roomState.armySelection.guestReady ? "ready" : "choosing"}. Their distributions remain private until deployment.`;
    elements.armyYouName.textContent = roomState.players.host.name ?? "Host";
    elements.armyYouReady.textContent = roomState.armySelection.hostReady ? "Locked" : "Choosing";
    elements.armyOpponentName.textContent = roomState.players.guest.name ?? "Guest";
    elements.armyOpponentReady.textContent = roomState.armySelection.guestReady ? "Locked" : "Choosing";
    return;
  }
  ensureArmyBuilder(scenario.size);
  elements.armyYouName.textContent = "You";
  const selection = roomState.armySelection;
  if (selection.youArmy) armyBuilder.setArmy(selection.youArmy);
  armyBuilder.setDisabled(selection.youReady || busy);
  elements.armyLimitHeading.textContent = `Build up to ${scenario.size === 16 ? 32 : 16} units`;
  elements.armySelectionCopy.textContent = selection.youReady
    ? "Your army is locked and hidden while your opponent finishes choosing."
    : "Your opponent cannot see this distribution until both armies are locked and deployment begins.";
  elements.lockArmy.hidden = selection.youReady;
  elements.lockArmy.disabled = Boolean(armyDraftError) || busy;
  elements.withdrawArmy.hidden = !selection.youReady;
  elements.armyYouReady.textContent = selection.youReady ? "Locked" : "Choosing";
  elements.armyOpponentName.textContent = roomState.players.opponent.name ?? "Opponent";
  elements.armyOpponentReady.textContent = selection.opponentReady ? "Locked" : "Choosing";
  elements.armyWaitingCopy.textContent = selection.opponentReady && !selection.youReady
    ? "Your opponent is ready. Lock a valid army to begin deployment."
    : "Deployment begins automatically after both commanders lock a valid force.";
}
...
function handleBattleSquare(game, x, y) {
  if (busy || roomState.phase === "ended" || roomState.seat === "spectator") return;
...
function renderBattleBoard(game) {
  ...
  const chosen = spectator ? null : selectedUnit(game);
...
function battleStatus() {
  if (roomState.seat === "spectator") {
    if (roomState.phase === "deployment") {
      const commander = roomState.deploymentTurn === "host" ? roomState.players.host.name : roomState.players.guest.name;
      return `${commander ?? "A commander"} is placing the next unit. Spectator mode is read-only.`;
    }
    if (roomState.phase === "ended") return roomState.result?.reason ?? "The battle has ended.";
    if (roomState.readiness.host && roomState.readiness.guest) return "Both commanders committed. The round is resolving.";
    if (roomState.readiness.host) return `${roomState.players.host.name} has committed; waiting for ${roomState.players.guest.name}.`;
    if (roomState.readiness.guest) return `${roomState.players.guest.name} has committed; waiting for ${roomState.players.host.name}.`;
    return "Both commanders are preparing secret orders. Their choices remain hidden until resolution.";
  }
  if (roomState.phase === "deployment") {
...
function renderBattle() {
  ...
  elements.opponentBattleName.textContent = spectator
    ? `${roomState.players.host.name} vs ${roomState.players.guest.name} · Spectator mode`
    : `Opponent · ${roomState.players.opponent.name}`;
  ...
  elements.deploymentPanel.hidden = spectator || roomState.phase !== "deployment";
  elements.ordersPanel.hidden = spectator || !["orders", "ended"].includes(roomState.phase);
  elements.spectatorPanel.hidden = !spectator;
  elements.resign.hidden = spectator || roomState.phase === "ended";
  if (!spectator && roomState.phase === "deployment") {
    renderReserve(game);
  ...
  const youReady = spectator ? roomState.readiness.host : roomState.youReady;
  const opponentReady = spectator ? roomState.readiness.guest : roomState.opponentReady;
  elements.youReady.textContent = youReady ? "Committed" : "Preparing";
  elements.youReady.classList.toggle("ready", youReady);
  elements.opponentReadyName.textContent = spectator ? (roomState.players.guest.name ?? "Guest") : (roomState.players.opponent.name ?? "Opponent");
  elements.opponentReady.textContent = opponentReady ? "Committed" : "Preparing";
  elements.opponentReady.classList.toggle("ready", opponentReady);
  elements.youReady.parentElement.querySelector("span").textContent = spectator ? (roomState.players.host.name ?? "Host") : "You";
  ...
  elements.humanControlLabel.innerHTML = spectator ? '<i class="human-control"></i>Blue: host control' : '<i class="human-control"></i>Blue: your control';
  elements.computerControlLabel.innerHTML = spectator ? '<i class="computer-control"></i>Red: guest control' : '<i class="computer-control"></i>Red: opponent control';
  elements.perspectiveNote.textContent = spectator ? "The board uses the host’s orientation." : "Your army is always displayed at the bottom.";
  if (spectator) elements.spectatorStatusCopy.textContent = battleStatus();
  ...
  if (roomState.phase === "ended" && roomState.result) {
    ...
    if (resultKey !== shownResultKey) {
      ...
      const winner = spectator ? null : canonicalWinnerForSeat(roomState.result.winner, roomState.seat);
      const title = spectator
        ? roomState.result.winner === "draw"
          ? "Neither army prevails."
          : `${roomState.result.winner === PLAYERS.HUMAN ? roomState.players.host.name : roomState.players.guest.name} wins the battle.`
        : winner === "you" ? "The field is yours." : winner === "draw" ? "Neither army prevails." : "Your formation has broken.";
...
function render() {
  ...
  const spectatorNeedsActivation = credentials?.seat === "spectator" && !spectatorActivated;
  document.body.dataset.screen = roomState?.game && ["deployment", "orders", "ended"].includes(roomState.phase) && !manualEditorGame
    ? "battle"
    : "online";
  elements.entry.hidden = Boolean(roomState && !guestNeedsName && !spectatorNeedsActivation) || Boolean(manualEditorGame);
  elements.room.hidden = !roomState || guestNeedsName || spectatorNeedsActivation || !["lobby", "proposal"].includes(roomState.phase) || Boolean(manualEditorGame);
  elements.armyScreen.hidden = !roomState || guestNeedsName || spectatorNeedsActivation || roomState.phase !== "armies" || Boolean(manualEditorGame);
  elements.terrainEditor.hidden = !manualEditorGame;
  elements.battle.hidden = !roomState || spectatorNeedsActivation || !["deployment", "orders", "ended"].includes(roomState.phase) || Boolean(manualEditorGame);
  if (!roomState || guestNeedsName || spectatorNeedsActivation) renderEntry();
  if (manualEditorGame) renderManualEditor();
  else if (roomState && ["lobby", "proposal"].includes(roomState.phase)) renderRoom();
  else if (roomState?.phase === "armies") renderArmySelection();
...
function bindEvents() {
  ...
  elements.spectateForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    if (busy || !credentials) return;
    setBusy(true);
    try {
      spectatorActivated = true;
      saveCredentials({ ...credentials, spectatorActive: true });
      api.setCredentials(credentials);
      ...
      spectatorActivated = false;
      message("The spectator invitation could not be opened", error.message);
      renderEntry();
    } finally {
      setBusy(false);
    }
  ...
  elements.copyInvite.addEventListener("click", async () => {
    try {
      ...
      setTimeout(() => { elements.copyInvite.textContent = "Copy opponent link"; }, 1600);
    } catch {
  ...
  elements.copySpectator.addEventListener("click", async () => {
    try {
      await navigator.clipboard.writeText(elements.spectatorLink.value);
      elements.copySpectator.textContent = "Copied";
      setTimeout(() => { elements.copySpectator.textContent = "Copy spectator link"; }, 1600);
    } catch {
      elements.spectatorLink.select();
      message("Copy the spectator invitation", "The link is selected. Press Ctrl+C to copy it.");
    }
  });
  ...
  elements.lockArmy.addEventListener("click", () => {
    const scenario = getScenario(roomState.proposal.scenarioId);
    const error = armyValidationError(armyDraft, scenario.size);
    if (error) {
      message("The army is not ready", error);
      return;
    }
    runAction("select_army", { army: armyDraft });
  });
  elements.withdrawArmy.addEventListener("click", () => runAction("withdraw_army"));
...
async function initialize() {
  ...
  if (credentials.seat === "spectator" && !credentials.spectatorActive) {
    spectatorActivated = false;
    api.setCredentials(credentials);
    renderEntry();
    return;
  }
  spectatorActivated = credentials.seat === "spectator";
  api.setCredentials(credentials);
  setConnection("connecting");
  try {
    const result = await api.state();
