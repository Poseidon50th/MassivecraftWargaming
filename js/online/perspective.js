...
export function proposalForSeat(proposal, seat) {
  if (!proposal || seat === "host") return proposal ? structuredClone(proposal) : null;
...
export function placementToCanonical({ unitId, x, y }, seat, size) {
  if (seat === "host") return { unitId, x, y };
...
function viewPlayer(player, seat) {
  return seat === "host" ? player : flipPlayer(player);
}

function viewPoint(point, seat, size) {
  return seat === "host" ? point : rotatePoint(point, size);
}
...
function describeOrders(orders, seat, size) {
  ...
  return orders.map((order) => {
    ...
    const facing = seat === "host" ? order.facing : OPPOSITE_FACING[order.facing];
