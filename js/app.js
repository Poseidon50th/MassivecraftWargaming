...
import { DIRECTIONS, FACINGS, PLAYERS, TERRAIN, UNIT_DEFS } from "./engine/constants.js";
...
import { clearTerrain, isStartingRow, PLACEABLE_TERRAIN, randomizeTerrain, setTerrain, terrainSummary } from "./engine/terrain.js";
  ...
  for (let y = 0; y < 5; y += 1) {
    for (let x = 0; x < 5; x += 1) {
      ...
      if (x === practice.unit[0] && y === practice.unit[1]) {
        cell.innerHTML = `<span class="practice-token"><img src="${UNIT_ASSETS[type]}" alt="Your ${UNIT_DEFS[type].name}" /></span>`;
      }
      if (x === practice.target[0] && y === practice.target[1]) {
        cell.classList.add("practice-target");
        cell.innerHTML = `<span class="practice-token enemy"><img src="${UNIT_ASSETS[practice.targetType]}" alt="Enemy ${UNIT_DEFS[practice.targetType].name}" /></span>`;
      }
      board.append(cell);
    }
  }
...
function tutorialOrderCheck(order) {
  ...
  if (!plan) {
    return {
      ...
      text: "That order is permitted and has been recorded. It does not follow the recommended capture line, so inspect the control totals before locking or press Show hint. You may keep it, change it, or continue experimenting.",
    };
  }
...
function tutorialHintText() {
  const remaining = tutorialExpectedOrders().filter(
    (plan) => !currentHumanOrders().some((order) => orderMatchesPlan(order, plan)),
  );
  if (!tutorialExpectedOrders().length) {
    const complete = new Set(game.tutorial?.contributors ?? []);
    const missing = game.units.filter((unit) => unit.player === PLAYERS.HUMAN && !complete.has(unit.id));
    return missing.length
      ...
      : "Every unit has contributed to a capture. Finish constricting the remaining enemy positions.";
  }
...
function startGame(scenarioId, terrainMode = "random") {
  ...
  pendingMove = null;
  transientStatus = terrainMode === "manual"
...
function paintTerrain(x, y) {
  ...
  setTerrain(game, x, y, selectedTerrainType);
  transientStatus = `${TERRAIN[game.terrain[y][x]].name} placed on ${boardCoordinate(x, y)}. Continue painting or save the field.`;
  saveGame();
  renderBoard();
  renderTerrainPalette();
}
...
function finishTerrainPlacement() {
  ...
  transientStatus = "Terrain saved. Select a reserve unit, then place it in either of your back rows.";
  saveGame();
  render();
}
...
function setOrder(order) {
  ...
  if (tutorialCheck.advisory) {
    window.setTimeout(() => showTutorialFeedback(tutorialCheck.title, tutorialCheck.text), 30);
  }
}
...
function renderTutorialPanel() {
  ...
  const roundsRemaining = Math.max(0, (game.tutorial?.roundCap ?? 20) - game.round + 1);
  elements.tutorialRoundTitle.textContent = copy?.title ?? `Round ${game.round} · Adapt the formation`;
  elements.tutorialCoachText.textContent = game.tutorial?.hintVisible
    ? tutorialHintText()
    : (copy?.coach ?? `The prepared line has changed. You may continue freely; ${roundsRemaining} round${roundsRemaining === 1 ? " remains" : "s remain"}.`);
...
function lockOrders() {
  ...
  render();
  window.setTimeout(() => {
    const resolution = resolveRound(game, humanOrders, computerOrders);
    ...
    if (result) finishGame(
      tutorialRound
        ? { ...result, reason: `${TUTORIAL_ROUND_COPY[tutorialRound]?.result ?? "The battle ended after you departed from the recommended line."} ${result.reason}` }
        : result,
    );
    else {
      if (tutorialRound) {
        transientStatus = TUTORIAL_ROUND_COPY[game.round]?.coach ?? transientStatus;
        showTutorialFeedback(`Round ${tutorialRound} complete`, TUTORIAL_ROUND_COPY[tutorialRound]?.result ?? "Your order was resolved. Review the capture badges and adjust the formation for the next round.");
      }
      saveGame();
      render();
    }
...
elements.form.addEventListener("submit", (event) => {
  ...
  startGame(data.get("scenario"), data.get("terrainMode"));
});
elements.finishTerrainButton.addEventListener("click", finishTerrainPlacement);
elements.clearTerrainButton.addEventListener("click", () => {
  clearTerrain(game);
  transientStatus = "All special terrain cleared. Continue painting or save the empty field.";
  saveGame();
  render();
});
