...
import { DIRECTIONS, FACINGS, PLAYERS, UNIT_DEFS, rotateFacing } from "./engine/constants.js";
...
const SAVE_KEY = "oscird-standard-battle-v1";
...
function saveGame() {
  ...
  elements.continueButton.hidden = !localStorage.getItem(SAVE_KEY);
}
  ...
  for (let y = 0; y < game.size; y += 1) {
    for (let x = 0; x < game.size; x += 1) {
      ...
      if (unit) {
        ...
        token.innerHTML = `<span class="facing">${DIRECTIONS[unit.facing].glyph}</span>${UNIT_DEFS[unit.type].short}`;
        square.append(token);
        ...
        square.setAttribute("aria-label", `${String.fromCharCode(65 + x)}${game.size - y}, empty ${game.terrain[y][x]}`);
      }
...
function renderReserve() {
  ...
  for (const [type, count] of Object.entries(groups)) {
    ...
    button.innerHTML = `<strong>${UNIT_DEFS[type].short} ${UNIT_DEFS[type].name}</strong><span>×${count}</span>`;
...
function render() {
  ...
  elements.controlToggle.textContent = showControl ? "Control visible" : "Control hidden";
...
elements.rotateButton.addEventListener("click", () => {
  const unit = selectedUnit();
  if (!unit) return;
  const existing = currentHumanOrders().find((order) => order.unitId === unit.id && order.kind === "rotate");
  setOrder(rotationOrder(unit, rotateFacing(existing?.facing ?? unit.facing)));
});
