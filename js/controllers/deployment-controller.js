...
import { chooseComputerArmy, doctrineForDifficulty } from "../ai/roster.js";
import { armyTotal, validatedArmy } from "../data/armies.js";
import { getScenario } from "../data/scenarios.js";
import { PLAYERS, TERRAIN } from "../engine/constants.js";
import { createGame, isDeploymentSquare, placeUnit, replaceReserve, unitAt } from "../engine/model.js";
...
export function createDeploymentController({
  ...
  function chooseComputerReserve(game) {
    const humanArmy = reserveGroupsForPlayer(game, PLAYERS.HUMAN);
    const army = chooseComputerArmy({
      size: game.size,
      terrain: game.terrain,
      opponentArmy: humanArmy,
      difficultyId: game.aiDifficulty,
    });
    replaceReserve(game, PLAYERS.COMPUTER, army);
    game.aiDoctrine = doctrineForDifficulty(game.aiDifficulty).name;
  }

  function reserveGroupsForPlayer(game, player) {
    const counts = {};
    for (const unit of game.reserves[player]) counts[unit.type] = (counts[unit.type] ?? 0) + 1;
    return counts;
  }

  function startGame(scenarioId, terrainMode = "random", aiDifficulty = "captain", requestedHumanArmy = null) {
    cancelDelayedWork();
    const scenario = getScenario(scenarioId);
    const humanArmy = validatedArmy(requestedHumanArmy ?? scenario.army, scenario.size);
    const game = createGame(scenario, { human: humanArmy, computer: {} });
    ...
    if (terrainMode !== "manual") chooseComputerReserve(game);
    ...
    setStatus(terrainMode === "manual"
      ...
      : `Terrain randomized. Your ${armyTotal(humanArmy)}-unit army is ready; select a reserve unit and place it in either back row.`);
    save();
    openGame();
  }
  ...
  function finishTerrainPlacement() {
    const game = getGame();
    if (!game.reserves[PLAYERS.COMPUTER].length) chooseComputerReserve(game);
  ...
  function placeHumanUnit(x, y) {
    ...
    while (!game.reserves.human.length && game.reserves.computer.length) {
      if (!deployComputerUnit(game, game.aiDifficulty)) break;
    }
