...
function startGame(scenarioId, terrainMode = "random", difficulty = "captain") {
  deploymentController.startGame(scenarioId, terrainMode, difficulty);
}
...
function updateDifficultySelection() {
  ...
  const ready = Boolean(selected && (selected.value !== "codex" || confirmedCodex));
  elements.beginDeploymentButton.disabled = !ready;
  elements.difficultyRequiredNote.classList.toggle("ready", ready);
  elements.difficultyRequiredNote.textContent = ready
    ? `${difficultyDisplay(selected.value)} selected.`
...
elements.form.addEventListener("submit", (event) => {
  ...
  startGame(data.get("scenario"), data.get("terrainMode"), difficulty);
});
