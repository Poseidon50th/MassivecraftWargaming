import { PLAYERS, TERRAIN, UNIT_DEFS, facingsForUnit } from "../engine/constants.js";
import { DEFAULT_AI_DIFFICULTY, isDifficulty } from "../ai/difficulties.js";

export const SAVE_VERSION = 4;
export const SAVE_KEY = "massivecraft-wars-standard-battle-v4";
export const PREVIOUS_SAVE_KEYS = Object.freeze([
  "massivecraft-wars-standard-battle-v3",
...
function validateUnit(unit, size, ids, occupied) {
  ...
  assert(facingsForUnit(unit).includes(unit.facing), "A unit has an invalid facing.");
...
function normalizeAiState(value, computerIds, round) {
  const state = isRecord(value) ? value : {};
  const counts = (candidate) => Object.fromEntries(
    Object.entries(isRecord(candidate) ? candidate : {})
      .filter(([id, count]) => computerIds.has(id) && isIntegerInRange(count, 0, 100000)),
  );
  const rounds = (candidate) => Object.fromEntries(
    Object.entries(isRecord(candidate) ? candidate : {})
      .filter(([id, valueRound]) => computerIds.has(id) && isIntegerInRange(valueRound, 0, round)),
  );
  return {
    orderCounts: counts(state.orderCounts),
    moveCounts: counts(state.moveCounts),
    lastOrderedRound: rounds(state.lastOrderedRound),
    lastMovedRound: rounds(state.lastMovedRound),
    lastAdvancedRound: isIntegerInRange(state.lastAdvancedRound, 0, round) ? state.lastAdvancedRound : 0,
  };
}
...
export function migrateSave(candidate) {
  ...
  assert([1, 2, 3, SAVE_VERSION].includes(sourceVersion), "The save file uses an unsupported version.");
  ...
  if (sourceVersion < SAVE_VERSION) {
    ...
    migrated.aiState = migrated.aiState ?? {
      orderCounts: {},
      moveCounts: {},
      lastOrderedRound: {},
      lastMovedRound: {},
...
export function validateSave(candidate) {
  ...
  const computerIds = new Set(save.units.filter((unit) => unit.player === PLAYERS.COMPUTER).map((unit) => unit.id));
  save.aiState = normalizeAiState(save.aiState, computerIds, save.round);
