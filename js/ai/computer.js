...
export function planComputerTurn(game, requestedDifficulty = game.aiDifficulty) {
  ...
  for (const candidate of candidates) {
    ...
    const lateGeneralNoise = difficulty.id === "general" && board.round > 15 ? 7 : difficulty.noise;
...
function basicPlacementScore(game, reserve, x, y, difficultyId) {
  ...
  const flankPreference = reserve.type === "cavalry" ? centerDistance * 2.5 : -centerDistance * 2;
  const complementary = allies.filter((unit) => unit.type !== reserve.type && Math.abs(unit.x - x) <= 1).length;
  const forward = allies.filter((unit) => Math.abs(unit.x - x) <= 1 && unit.y > y).length;
  const rear = allies.filter((unit) => Math.abs(unit.x - x) <= 1 && unit.y < y).length;
  const screen = ranged ? forward : rear;
  const artilleryClusterPenalty = reserve.type === "artillery" && artilleryAdjacent(game, x, y) ? 90 : 0;
  if (difficultyId === "corporal") {
    return -desiredDepth + flankPreference + adjacent * 2 + complementary * 3 - sameColumn - artilleryClusterPenalty;
  }
  if (difficultyId === "captain") return desiredDepth + flankPreference + adjacent * 4 + complementary * 5 + screen * 5 - sameColumn - artilleryClusterPenalty;
  return desiredDepth + flankPreference + adjacent * (difficultyId === "codex" ? 11 : 8)
    + complementary * (difficultyId === "codex" ? 9 : 7) + screen * (difficultyId === "codex" ? 12 : 8)
    - sameColumn - artilleryClusterPenalty;
}
...
export function planComputerDeployment(game, requestedDifficulty = game.aiDifficulty) {
  ...
  const scored = finalists.flatMap((candidate) => placementFacings(candidate.reserve.type, difficulty.id).map((facing) => ({
    ...
    score: strategicPlacementScore(board, candidate.reserve, candidate.x, candidate.y, facing, difficulty.id)
      + jitter(random, difficulty.id === "general" ? 1 : difficulty.noise * 0.2),
  })));
