...
Version 0.7.2 adds a browser compatibility hotfix to the quota-hardened private, invite-link multiplayer introduced in v0.7.1, without removing or weakening the certified v0.6.1 single-player game.
...
1. recognizes the host and guest through separate high-entropy invitation tokens;
2. stores only token hashes;
3. validates battlefield proposals, terrain, deployments, facing, movement, and complete order sets;
4. keeps submitted orders out of every public room snapshot;
...
Start with [MULTIPLAYER_SETUP_GUIDE.md](MULTIPLAYER_SETUP_GUIDE.md). The implementation and maintenance details are in [MULTIPLAYER_ARCHITECTURE.md](MULTIPLAYER_ARCHITECTURE.md).
...
Run a live two-client WebSocket deployment and resolved round on both board sizes with:
...
Room invitations are capability links. Anyone possessing the guest invitation can claim that one guest seat, so players should send it privately. The token is placed after the URL’s `#`, preventing the browser from sending it to GitHub Pages in the page request. The browser retains its own seat token locally for reconnection; the server stores only its SHA-256 hash.

The server never accepts client-supplied path or terrain effects at face value. It re-finds the unit, rebuilds movement from the current board, recalculates River penalties, validates facing and destinations, and rejects incomplete or stale order sets. Public snapshots contain readiness only—not submitted orders.

The computer opponent also receives a detached public board without the player’s prepared orders. Aristides’ Codex is intentionally deterministic and searches the strongest predicted replies available within its bounded browser budget; it does not receive foreknowledge of current player orders.
...
js/data/                                Scenarios and tutorial data
...
js/online/perspective.js                Host/guest board rotation and dispatch perspective
js/online/online-controller.js          Multiplayer lobby, field editor, deployment, and orders UI
