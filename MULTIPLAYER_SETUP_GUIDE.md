...
**Order matters:** deploy and check the Cloudflare server first. Paste its address into the website second. Upload the website to GitHub last. Do not publish the v0.7.2 website while an older server is still running.

**Already deployed v0.7.1?** The v0.7.2 fix is entirely in the website. If your existing `/api/health` page already shows `"protocolVersion":2` and `"quotaHardened":true`, do not redeploy or delete the Cloudflare Worker. Keep its current address in `js/online/config.js` and replace the website files on GitHub.
...
v0.7.2 is deliberately conservative with that free allowance:
...
1. Download `massivecraft-wars-v0.7.2.zip`.
2. Open your **Downloads** folder.
3. Right-click the ZIP.
4. Choose **Extract All…**.
5. Leave the suggested destination alone.
6. Click **Extract**.
7. Open the new `massivecraft-wars-v0.7.2` folder.
...
1. Return to the extracted `massivecraft-wars-v0.7.2` folder.
...
The first v0.7.2 deployment creates two SQLite-backed Durable Object classes:
...
If you see all three, the correct v0.7.2-compatible server is alive. If it says protocol version 1, stop: the old v0.7.0 Worker is still deployed.
...
## Part 9 — Upload v0.7.2 to GitHub

Do this part only after Part 7 showed `"protocolVersion":2` and Part 8 saved the Worker address.

Upload the **contents** of the extracted `massivecraft-wars-v0.7.2` folder into the root of `Poseidon50th/MassivecraftWargaming`, replacing matching files as you did for the earlier releases.
...
1. Open:
   ...
   `https://poseidon50th.github.io/MassivecraftWargaming/?v=0.7.2`

2. Confirm that the page says `v0.7.2`.
...
The v0.7.2 safety budget reached 400 room creations since midnight UTC. Battles already in progress keep priority. Wait until after midnight UTC and create the room again. Do not delete the Worker or either Durable Object namespace.
...
Confirm the repository contains v0.7.2, wait for the Pages Action, open the `?v=0.7.2` link, and press `Ctrl+F5`.
...
When upgrading from v0.7.0 specifically, deploy the v0.7.2 Worker first. Stored v0.7.0 rooms migrate when reopened, and the old website can continue using the retained HTTP route during the short deployment gap. After the v0.7.2 health check passes, upload the v0.7.2 website.
