...
Massivecraft’s Wars v0.8.0 keeps the game itself on GitHub Pages while using an authoritative, quota-hardened room service for two players and read-only spectators. The service keeps army choices and orders private at the correct phases, enforces the shared rules, survives refreshes, and protects ongoing matches from wasteful transport or room spam.
...
| Online controller | Creates or joins rooms, activates read-only spectator mode, renders the correct perspective, holds only local unsubmitted drafts, and sends authorized actions. |
...
| Proposal | Proposes random terrain or a complete manual field. | Approves or requests a different field. | Approval opens private army selection. |
| Armies | Privately builds and locks a legal force. | Privately builds and locks a legal force. | Both locks create the game and reveal reserves for deployment. |
...
Spectators may enter any phase through the separate spectator capability link. They can inspect the field, public positions, connection/readiness state, resolved reports, and final result. They cannot claim a seat, approve a field, submit an army, place a unit, prepare an order, withdraw an order, or resign.
...
- whether each side has committed orders;
- whether each side has locked an army, without its distribution until both are locked; and
- structured reports from already resolved rounds.

The client never receives:

- the other player’s token or stored token hash;
- either player’s private army distribution before both commanders lock;
...
The canonical server uses `human` for the host and `computer` for the guest because those are the engine’s stable side identifiers. The guest browser rotates every coordinate and facing by 180 degrees and swaps the display colors. Consequently, both players see their own blue army at the bottom while all server actions still reference one canonical board. Spectators use the host’s canonical orientation, with blue labeled Host and red labeled Guest.

## Army validation

The server validates the entire distribution rather than trusting browser controls. Counts must be non-negative whole numbers, unknown unit types are rejected, an army must contain at least one unit, 8 × 8 rooms allow at most 16 units, 16 × 16 rooms allow at most 32, and either size allows no more than two Artillery. Armies remain private while the room is in the `armies` phase. Unequal army sizes are supported; once one reserve is empty, the other commander receives consecutive remaining placements.
...
The invitation token is stored in the browser for that room and seat. A refresh requests one sanitized snapshot, then opens a hibernatable WebSocket. Battlefield approval, private army locking, deployment, orders, withdrawal, resignation, state synchronization, spectator delivery, and reconnect delivery all use that socket. There is no continuous polling loop.
...
- Public room protocol: `3`
- Durable Object migration tag: `v1`
- Admission Durable Object migration tag: `v2`
- Game release: `0.8.0`

Do not remove or rename either Durable Object class or either deployment migration when publishing an ordinary update. Stored protocol-1 and protocol-2 rooms migrate safely to protocol 3 when first reopened. Rooms created by a cached v0.7.2 page retain the legacy direct-approval flow; v0.8.0 clients identify protocol 3 at room creation and receive the army-muster phase. If the stored room shape changes later, extend the explicit room migration function rather than silently reinterpreting old state.
