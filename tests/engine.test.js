...
import { formationPlanFeatures, formationPlanScore } from "../js/ai/candidate-orders.js";
import { chooseComputerArmy, doctrineForDifficulty } from "../js/ai/roster.js";
import { sanitizePublicBoard } from "../js/ai/public-board.js";
import { armyTotal, armyValidationError } from "../js/data/armies.js";
...
test("custom armies enforce board limits and the two-Artillery maximum", () => {
  assert.equal(armyValidationError({ cavalry: 14, artillery: 2 }, 8), null);
  assert.match(armyValidationError({ cavalry: 14, artillery: 3 }, 8), /at most 16|at most 2 Artillery/);
  assert.equal(armyValidationError({ sword: 1 }, 16), null, "players may deliberately field fewer than the maximum");
  const custom = createGame(getScenario("skirmish"), {
    human: { cavalry: 14, artillery: 2 },
    computer: { sword: 8, spear: 8 },
  });
  assert.equal(custom.reserves.human.length, 16);
  assert.equal(custom.reserves.human.filter((unit) => unit.type === "artillery").length, 2);
  assert.equal(custom.reserves.computer.length, 16);
});

test("every computer chooses a full legal doctrine-specific roster", () => {
  ...
  for (const difficulty of ["corporal", "captain", "general", "codex"]) {
    const army = chooseComputerArmy({
      size: 8,
      terrain,
      opponentArmy: { sword: 16 },
      difficultyId: difficulty,
    });
    assert.equal(armyTotal(army), 16, `${difficulty} should use the full force allowance`);
    assert.ok(army.artillery <= 2, `${difficulty} exceeded the Artillery limit`);
    assert.equal(armyValidationError(army, 8), null);
    assert.ok(doctrineForDifficulty(difficulty).name);
    armies.set(difficulty, JSON.stringify(army));
  }
  assert.ok(new Set(armies.values()).size >= 3, "the commanders should not all select the same distribution");

  const large = chooseComputerArmy({ size: 16, terrain: Array.from({ length: 16 }, () => Array(16).fill("plain")), difficultyId: "codex" });
  assert.equal(armyTotal(large), 32);
  assert.ok(large.artillery <= 2);
});

test("computer deployment always completes a legal full army with two Artillery", () => {
  const scenario = getScenario("skirmish");
  const game = createGame(scenario, {
    human: { sword: 16 },
    computer: { sword: 14, artillery: 2 },
  });
  while (game.reserves.human.length) {
    const reserve = game.reserves.human[0];
    let square = null;
    for (let y = game.size - game.deploymentRows; y < game.size && !square; y += 1) {
      for (let x = 0; x < game.size; x += 1) {
        if (!unitAt(game, x, y)) {
          square = { x, y };
          break;
    ...
    placeUnit(game, reserve, square.x, square.y, "north");
    assert.ok(deployComputerUnit(game, "captain"));
  }
  assert.equal(game.reserves.computer.length, 0);
  assert.equal(activeUnits(game, "computer").filter((unit) => unit.type === "artillery").length, 2);
});

test("formation planning prefers a supported advance over an isolated bait move", () => {
  const game = gameWith([
    { id: "computer-sword", player: "computer", type: "sword", x: 3, y: 2, facing: "south" },
    { id: "computer-spear", player: "computer", type: "spear", x: 2, y: 2, facing: "south" },
    { id: "human-axe", player: "human", type: "axe", x: 3, y: 6, facing: "north" },
  ]);
  const supported = [{ kind: "move", unitId: "computer-sword", to: { x: 3, y: 3 }, path: [{ x: 3, y: 3 }], facing: "south" }];
  const isolated = [{ kind: "move", unitId: "computer-sword", to: { x: 5, y: 2 }, path: [{ x: 4, y: 2 }, { x: 5, y: 2 }], facing: "south" }];
  const supportedFeatures = formationPlanFeatures(game, PLAYERS.COMPUTER, supported);
  assert.equal(supportedFeatures.supportedMoves, 1);
  assert.ok(formationPlanFeatures(game, PLAYERS.COMPUTER, isolated).unsupportedVanguard >= 1);
  for (const difficulty of ["corporal", "captain", "general", "codex"]) {
    assert.ok(
      formationPlanScore(game, PLAYERS.COMPUTER, supported, difficulty)
        > formationPlanScore(game, PLAYERS.COMPUTER, isolated, difficulty),
      `${difficulty} should recognize the supported formation`,
    );
  }
});
...
test("16 × 16 planning uses bounded beam search", () => {
  ...
  assert.equal(plan.diagnostics.timeBudgetMs, 14000);
  assert.ok(plan.diagnostics.elapsedMs <= 14500);
...
test("deployment doctrines remain distinct while every opening faces the field", () => {
  ...
  assert.equal(corporal.facing, "south");
  assert.equal(codex.facing, "south");
  assert.ok(codex.y >= corporal.y, "Codex should form its active screen no farther back than the Corporal's first placement");
});
