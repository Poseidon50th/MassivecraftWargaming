...
test("the browser fetch implementation remains bound to its global object", async () => {
  function browserStyleFetch(url, options) {
    ...
    assert.deepEqual(JSON.parse(options.body), { hostName: "Host", clientProtocol: 3 });
...
test("browser timer implementations remain bound to their global object", async () => {
  function browserStyleSetTimeout(callback) {
    assert.equal(this, globalThis);
    callback();
    return 17;
  }
  function browserStyleClearTimeout(timer) {
    assert.equal(this, globalThis);
    assert.equal(timer, 17);
  }
  const api = new MultiplayerApi("https://worker.example", {
    fetchImpl: async () => new Response(JSON.stringify({ roomId: "ROOM12345678" }), {
      status: 201,
      headers: { "content-type": "application/json" },
    }),
    WebSocketImpl: null,
    setTimeoutImpl: browserStyleSetTimeout,
    clearTimeoutImpl: browserStyleClearTimeout,
  });
  const result = await api.createRoom("Host");
  assert.equal(result.roomId, "ROOM12345678");
  api.close();
