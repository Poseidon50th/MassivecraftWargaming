...
test("the guided tutorial includes unit practices, hints, mistake guidance, and a 20-round cap", () => {
  ...
  assert.match(app, /UNIT_TUTORIALS/);
  assert.match(app, /Order accepted/);
  assert.match(app, /roundCap/);
  assert.match(app, /startTutorialBattle/);
});

test("terrain selection, manual painting, random placement, and hover rules are present", () => {
  assert.match(html, /name="terrainMode" value="random"/);
  assert.match(html, /name="terrainMode" value="manual"/);
  assert.match(html, /id="terrain-panel"/);
  assert.match(html, /id="terrain-readout"/);
  assert.match(app, /randomizeTerrain/);
  assert.match(app, /paintTerrain/);
  assert.match(app, /makeTerrainLayer/);
  for (const terrain of ["wall", "hill", "mud", "river", "road", "fog"]) {
    assert.match(html + app, new RegExp(terrain, "i"));
  }
});
