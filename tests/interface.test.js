...
function giveMove(from, to, facing) {
  ...
  assert.ok(facingButton, "a moved piece should display its permitted facing choices on the board");
  facingButton.click();
}
...
test("round-three hints, tutorial lessons, battlefield orders, and cancelled resolution work through real clicks", async () => {
  ...
  boardSquare(2, 7).click();
  assert.equal(
    boardSquare(2, 7).querySelectorAll(".board-facing-choice").length,
    8,
    "Spears should show cardinal and diagonal facing arrows",
  );
  assert.ok(boardSquare(2, 7).querySelector(".board-facing-choice.northeast"));
  boardSquare(0, 7).click();
  assert.equal(
    boardSquare(0, 7).querySelectorAll(".board-facing-choice").length,
    4,
    "Swords should retain cardinal facing arrows",
  );

  giveMove({ x: 0, y: 7 }, { x: 0, y: 6 }, "north");
  giveMove({ x: 2, y: 7 }, { x: 2, y: 6 }, "northeast");
...
test("deployment remains locked until a difficulty is chosen and Codex requires confirmation", () => {
  ...
  assert.match(document.querySelector("#single-player-army-builder .army-total").textContent, /32 \/ 32/);
  assert.equal(document.querySelector('#single-player-army-builder input[aria-label="Artillery count"]').value, "2");
  click('input[name="scenario"][value="skirmish"]');
  assert.match(document.querySelector("#single-player-army-builder .army-total").textContent, /16 \/ 16/);
