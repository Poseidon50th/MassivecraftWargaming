...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname === "/api/health") {
      send(response, 200, { ok: true, service: "Local Massivecraft test server", protocolVersion: 2, quotaHardened: true }, origin);
      return;
    }
    if (request.method === "POST" && url.pathname === "/api/rooms") {
      ...
      rooms.set(roomId, { state, hostToken, guestToken });
      send(response, 201, { roomId, hostToken, guestToken, state: publicRoomSnapshot(state, "host") }, origin);
      return;
    }
