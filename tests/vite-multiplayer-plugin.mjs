      ...
      server.middlewares.use("/test-multiplayer", async (request, response) => {
        try {
          ...
          if (request.method === "GET" && url.pathname === "/api/health") {
            send(response, 200, { ok: true, service: "Local Massivecraft test server", protocolVersion: 2, quotaHardened: true });
            return;
          }
          if (request.method === "POST" && url.pathname === "/api/rooms") {
            ...
            const state = createInitialRoom({ roomId, hostName: data.hostName, hostTokenHash: "test-host", guestTokenHash: "test-guest", seed: 42, now: Date.now() });
            const record = { state, hostToken, guestToken, sockets: new Set() };
            rooms.set(roomId, record);
            send(response, 201, { roomId, hostToken, guestToken, state: publicRoomSnapshot(state, "host", connectedSeats(record)) });
            return;
          }
