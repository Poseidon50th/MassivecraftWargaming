...
function nextPlacement(state, player) {
  ...
  const reserve = game.reserves[player].find((unit) => unit.type === "artillery") ?? game.reserves[player][0];
...
async function certify(scenarioId, expectedUnits) {
  ...
  const spectator = new MultiplayerApi(serverUrl, { fetchImpl: countedFetch });
  ...
  spectator.setCredentials({ roomId: created.roomId, token: created.spectatorToken });
  let state = (await guest.state()).state;
  const spectatorState = (await spectator.state()).state;
  assert.equal(spectatorState.seat, "spectator");
  const hostConnected = connected(host);
  const guestConnected = connected(guest);
  const spectatorConnected = connected(spectator);
  host.connect();
  guest.connect();
  spectator.connect();
  await Promise.all([hostConnected, guestConnected, spectatorConnected]);
  ...
  assert.equal(state.phase, "armies");
  const size = scenarioId === "battle" ? 16 : 8;
  const hostArmy = defaultArmyForSize(size);
  const guestArmy = scenarioId === "battle" ? { cavalry: 30, artillery: 2 } : { sword: 14, artillery: 2 };
  state = (await host.action("select_army", { army: hostArmy })).state;
  const spectatorChoosing = (await spectator.sync()).state;
  assert.deepEqual(spectatorChoosing.armySelection, { hostReady: true, guestReady: false });
  assert.equal(JSON.stringify(spectatorChoosing).includes('"sword":4'), false, "spectators must not receive a private roster before deployment");
  state = (await guest.action("select_army", { army: guestArmy })).state;
  ...
  const watching = (await spectator.sync()).state;
  assert.equal(watching.readiness.host, true);
  assert.deepEqual(watching.game.orders, { human: [], computer: [] });
