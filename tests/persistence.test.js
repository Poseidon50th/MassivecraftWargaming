...
test("version-three saves receive validated AI activation state", () => {
  const legacy = createGame(getScenario("skirmish"));
  legacy.version = 3;
  delete legacy.aiState;
  const migrated = validateSave(legacy);
  assert.equal(migrated.version, SAVE_VERSION);
  assert.deepEqual(migrated.aiState, {
    orderCounts: {},
    moveCounts: {},
    lastOrderedRound: {},
    lastMovedRound: {},
...
test("save validation rejects corrupt terrain, duplicate units, and unknown versions", () => {
  ...
  const invalidDiagonal = buildTutorialGame();
  invalidDiagonal.units.find((unit) => unit.type === "sword").facing = "northeast";
  assert.throws(() => validateSave(invalidDiagonal), /invalid facing/i);

  const validDiagonal = buildTutorialGame();
  validDiagonal.units.find((unit) => unit.type === "spear").facing = "northeast";
  assert.doesNotThrow(() => validateSave(validDiagonal));
