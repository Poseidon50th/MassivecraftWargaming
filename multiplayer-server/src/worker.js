...
export class MatchRoom extends DurableObject {
  ...
  connectedSeats() {
    const spectators = this.ctx.getWebSockets("spectator").length;
    return {
  ...
  async identify(rawToken, request) {
    ...
    if (this.room.auth.spectator && constantTimeEqual(hashed, this.room.auth.spectator)) {
      this.authFailures.delete(key);
      return "spectator";
    }
    recent.push(now);
  ...
  async fetch(request, env) {
    try {
      ...
      if (request.method === "GET" && url.pathname === "/api/health") {
        return withCors(json({ ok: true, service: "Massivecraft's Wars Multiplayer", protocolVersion: 3, quotaHardened: true, spectators: true, customArmies: true }), request, env);
      }
      if (request.method === "POST" && url.pathname === "/api/rooms") {
        ...
        const state = await stub.initialize({
          ...
          spectatorTokenHash: await sha256(spectatorToken),
          armySelectionEnabled: Number(body.clientProtocol ?? 0) >= 3,
          seed: seedBytes[0] || 1,
          now: Date.now(),
        });
        return withCors(json({ roomId: id, hostToken, guestToken, spectatorToken, state }, 201), request, env);
      }
