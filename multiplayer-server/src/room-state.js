...
export function createInitialRoom({ roomId, hostName, hostTokenHash, guestTokenHash, seed = 1, now = Date.now() }) {
  return {
    ...
    auth: { host: hostTokenHash, guest: guestTokenHash },
    proposal: null,
    game: null,
    deploymentTurn: null,
...
export function publicRoomSnapshot(room, seat, connected = { host: false, guest: false }) {
  ...
  return {
    ...
    players: {
      you: { name: room.players[seat].name, connected: Boolean(connected[seat]) },
      opponent: {
        name: room.players[seat === "host" ? "guest" : "host"].name,
        connected: Boolean(connected[seat === "host" ? "guest" : "host"]),
      },
    },
    proposal: room.proposal ? structuredClone(room.proposal) : null,
    game: sanitizeGame(room.game),
    deploymentTurn: room.deploymentTurn === null ? null : room.deploymentTurn === player ? "you" : "opponent",
    youReady: Boolean(room.ready[player]),
    opponentReady: Boolean(room.ready[enemy]),
